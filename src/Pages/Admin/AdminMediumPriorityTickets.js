...
const AdminMediumPriorityTickets = () => {
    ...
    useEffect(() => {
        axios
            .get('http://localhost:3001/admin/mediumprioritytickets')
            .then((res => [
    ...
    return (
        <div className='d-flex bg-secondary-subtle' id='wrapper'>
            ...
            <div className='w-100' id='page-content-wrapper'>
                <NavbarAdmin />
                <h3 className='my-4 mx-2 p-2 rounded text-center bg-white text-warning shadow'>Medium Priority Tickets</h3>
                {detail.map((ticket, index) => (
                    <div key={index} className='  p-1 m-2 bg-white shadow rounded'>
                        <p className='fs-5 fw-bolder text-center bg-warning p-1 rounded'>Subject: {ticket.subject}</p>
...
export default AdminMediumPriorityTickets